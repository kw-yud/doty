...
local function checkRequirement(command, msg)
    if not fn.isExecutableAvailable(command) then
        fn.notify(string.format("Missing %s binary or not found in PATH - %s",
                                command, msg), vim.log.levels.WARN,
                  "config.preflight")
    end
end

for k, v in pairs(required_programs) do checkRequirement(k, v) end

if not fn.isNeovimVersionSupported('0.9') then
    fn.notify(string.format(
                  "This config probably won't work very well with Neovim < 0.9"),
              vim.log.levels.WARN, "config.preflight")
end
